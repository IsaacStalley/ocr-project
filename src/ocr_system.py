...
# Main class for text recognition system
class TextRecognitionSystem:
    def __init__(self):
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        # Initialize SSD model for word detection
        ...
        # Initialize CRNN model for text recognition
        ...
        num_class = len(self.LABEL2CHAR)  # + 1 if including blank label
    ...
    def process_image(self, image):
        ...
        orig_image = open_cv_image[:, :, ::-1].copy()  # Convert RGB to BGR
        image = cv2.cvtColor(orig_image, cv2.COLOR_BGR2RGB)
        
        # Use SSD predictor to get bounding boxes for words
        ...
        # Annotate original image with recognized words
    ...
    def predict(self, dataloader):
        # Perform prediction using CRNN model
        all_preds = []
        with torch.no_grad():
